(() => {
            ...
            function initSpollers(spollersArray, matchMedia = false) {
                spollersArray.forEach((spollersBlock => {
                    ...
                    if (matchMedia.matches || !matchMedia) {
                        ...
                        document.documentElement.classList.add("spoller-active");
                ...
                if (el.closest("[data-spoller]")) {
                    ...
                    if (!spollersBlock.querySelectorAll("._slide").length) {
                        if (oneSpoller && !spollerTitle.classList.contains("_spoller-active")) {
                            document.documentElement.classList.remove("spoller-active");
                            hideSpollersBody(spollersBlock);
                        }
                        document.documentElement.classList.toggle("spoller-active");
        ...
        function pageNavigationAction(e) {
            if ("click" === e.type) {
                ...
                if (targetElement.closest("[data-goto]")) {
                    ...
                    if (gotoBlock(gotoLinkSelector, noHeader, gotoSpeed, offsetTop)) e.preventDefault();
                }
    ...
    class Vote {
        ...
        get sumAll() {
            let allPoints = 0;
            let scored = 0;
            for (const point of this.points) allPoints += point;
            for (const card of this.cards) scored += parseInt(card.querySelector("[data-score]").dataset.score);
        ...
        init() {
            for (const card of this.cards) {
                ...
                for (const Num of Nums) Num.addEventListener("click", (event => {
                    if (!Num.classList.contains("avl")) {
                        for (const Num of Nums) Num.classList.contains("clicked") ? Num.classList.remove("clicked") : null;
                ...
                openNums.addEventListener("click", (event => {
                    for (const Num of Nums) Num.classList.contains("avl") ? Num.classList.remove("avl") : null;
                    for (const card of this.cards) {
                        if (currentCard != card) {
                            const score = card.querySelector("[data-score]");
                            if ("0" != score.innerText) for (const Num of Nums) if (Num.innerText == score.innerText) Num.classList.add("avl");
                        }
            ...
            if (submitBtn) submitBtn.addEventListener("click", (event => {
                if ("0" == this.avaliableLabel.innerText) this.toDB().then((data => {
        ...
        plus(currentCard, currentScore) {
            for (const card of this.cards) if (card !== currentCard) if (card.querySelector("[data-score]").dataset.score == currentScore) return false;
            return true;
        }
    }
